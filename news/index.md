...
- Session metadata is now stored in each bookmark pin’s metadata field
  instead of a separate shared “sessions” pin. This eliminates race
  conditions from concurrent writes. Legacy sessions from v0.1.0 and
  earlier are automatically migrated on first use.
- Added `reactive_sessions()` method to `StorageClass` for reactive
  bookmark lists that refresh based on triggers
- Added `validate_session_metadata()` to ensure metadata fields are
  scalar values
- Session timestamp is now automatically added by the system in ISO 8601
  format if not provided by the user

### Bug Fixes

- Fixed race condition where multiple concurrent bookmark saves could
  result in metadata loss and orphaned bookmark data
- In `examples-shiny`, fixed bookmark module example to properly refresh
  session list after save/delete operations
- In `examples-shiny`, fixed filter module to handle NULL parameter
  values during bookmark restoration
